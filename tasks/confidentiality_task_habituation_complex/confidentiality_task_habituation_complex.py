...
if settings_obj.run_session:
    ...
    for trial in range(settings_obj.trial_number):
        ...
        sides_li.append(
            probability_obj.stim_side_dict.copy()
        )  # put corect and wrong side in sides_li
        insist_mode_li.append(
            (probability_obj.insist_mode_active, probability_obj.insist_side)
        )  # put insist mode status in inist_mode_li
        ...
        try:
            ...
            if settings_obj.stim_type == "three-stimuli":
                print("three")
                stimulus_game.run_game_habituation_3_complex(
                    display_stim_event, still_show_event, bpod, sma
                )
            elif settings_obj.stim_type == "two-stimuli":
                print("tow")
                stimulus_game.run_game_habituation_2_complex(
                    display_stim_event, still_show_event, bpod, sma
                )
            else:
