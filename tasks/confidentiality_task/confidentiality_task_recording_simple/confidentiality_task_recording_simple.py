...
settings_folder = currentdir  # os.path.join(currentdir.split('experiments')[0],"tasks","confidentiality_task_recording_simple")
...
window.draw_window_bevore_conf(stage="training") #### check change to recording
window.show_window()
...
if settings_obj.run_session:
    ...
    for trial in range(settings_obj.trial_number):
        ...
        # reset rotary encoder before checking for wheel not stoping
        sma.add_state(
            ...
            output_actions=[("Serial1", settings_obj.RESET_ROTARY_ENCODER),
                            ('BNC1', 0), ('BNC2', 1)],   # activate white light while waiting
        )
        ...
        sma.add_state(
            ...
            output_actions=[('BNC1', 0), ('BNC2', 1)], ## check this extra ,
        )
        ...
        sma.add_state(
            ...
            state_timer=0,
            state_change_conditions={"Tup": "reset_rotary_encoder_wheel_stopping_check"},
            output_actions=[('BNC1', 0), ('BNC2', 0), ("SoftCode", 9)], ## check this extra ,
        )
        ...
        sma.add_state(
            ...
            output_actions=[("SoftCode", settings_obj.SC_PRESENT_STIM),
                            ('BNC1', 0), ('BNC2', 1)],  # after wait -> present initial stimulus
        )
        ...
        sma.add_state(
            ...
            output_actions=[('BNC1', 0), ('BNC2', 0)],  # after wait -> present initial stimulus
        )

        # reset rotary encoder befor open loop starts
        sma.add_state(
            state_name="reset_rotary_encoder_open_loop",
            state_timer=0,
            state_change_conditions={"Tup": "open_loop"},
            output_actions=[("Serial1", settings_obj.RESET_ROTARY_ENCODER),
                            ('BNC1', 0), ('BNC2', 1)],  # reset rotary encoder postition to 0
        )
        ...
        sma.add_state(
            ...
            output_actions=[("SoftCode", settings_obj.SC_START_OPEN_LOOP),
                            ('BNC1', 0), ('BNC2', 0)],  # softcode to start open loop
        )
        ...
        sma.add_state(
            ...
            output_actions=[("SoftCode", settings_obj.SC_STOP_OPEN_LOOP),
                            ('BNC1', 0), ('BNC2', 1)],  # stop open loop in py game
        )
        ...
        sma.add_state(
            ...
            output_actions=[('BNC1', 0), ('BNC2', 0)],  # stop open loop in py game
        )
        ...
        # extra end state for bnc1 and bnc2 = +1
        sma.add_state(
